...
for filename in pm_files:
    ...
    updated     = 0
    warnings    = 0
    chosen_rows = {}   # apex_id -> row number admin chose to keep (for duplicates)
    ...
    # Admin chooses which row to keep for each duplicate
    chosen_rows = {}   # apex_id -> row_num to use
    if dup_ids:
        print()
        print("  !" * 30)
        print(f"  CRITICAL WARNING in file: {filename}")
        print(f"  Duplicate APEX IDs found — you must choose which row to keep for each.")
        print("  !" * 30)
        print()
        for apex_id, r1, r2 in dup_ids:
            print(f"  APEX ID: {apex_id}")
            # Show values from both rows
            row1_data = pm_df[pm_df[UNIQUE_KEY] == apex_id].iloc[0]
            row2_data = pm_df[pm_df[UNIQUE_KEY] == apex_id].iloc[1]
            PREVIEW_COLS = ["Sub Format", "Site Name", "EPC Status (Current Week)"]
            print(f"    Row {r1} values (first occurrence):")
            for col in PREVIEW_COLS:
                v = clean(row1_data.get(col, ""))
                print(f"      {col}: {v if v else '(empty)'}")
            print(f"    Row {r2} values (second occurrence):")
            for col in PREVIEW_COLS:
                v = clean(row2_data.get(col, ""))
                print(f"      {col}: {v if v else '(empty)'}")
            print()
            while True:
                choice = input(f"  Keep row {r1} or row {r2}? Type {r1} or {r2}: ").strip()
                if choice == str(r1):
                    chosen_rows[apex_id] = r1
                    print(f"  -> Row {r1} will be used for APEX ID {apex_id}")
                    break
                elif choice == str(r2):
                    chosen_rows[apex_id] = r2
                    print(f"  -> Row {r2} will be used for APEX ID {apex_id}")
                    break
                else:
                    print(f"  Invalid input. Please type exactly {r1} or {r2}.")
            print()
            warning_log.append({"File": filename, UNIQUE_KEY: apex_id,
                                 "Issue": f"Duplicate — admin chose row {chosen_rows[apex_id]}"})
    ...
    for row_num, (_, row) in enumerate(pm_df.iterrows(), start=2):
        ...
        # ── Skip duplicate rows based on admin choice ───────────────────────
        if apex_id in chosen_rows:
            if row_num != chosen_rows[apex_id]:
                continue  # skip the row admin did not choose
        else:
            # No duplicate — but skip if already processed
...
# Add date and time to merged filename
master_name = MASTER_FILE.replace(".xlsx", "")
output_file = f"Updated_{master_name}_{stamp}.xlsx"
wb.save(output_file)
